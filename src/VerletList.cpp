...
/*-------------------------------------------------------------*/
  ...
  VerletList::VerletList(shared_ptr<System> system, real _cut, bool rebuildVL) : SystemAccess(system)
  {
    ...
    if (rebuildVL) {
	rebuild(); // not called if exclutions are provided
	rebuild2(); // add
    }
    ...
    connectionResort = system->storage->onParticlesChanged2.connect(
        boost::bind(&VerletList::rebuild2, this));
  }
  ...
  void VerletList::rebuild()
  {
    ...
    vlPairs1 = vlPairs;
    //std::copy(vlPairs2.begin(),vlPairs2.end(),std::back_inserter(vlPairs));//rebuild

    builds++;
    LOG4ESPP_DEBUG(theLogger, "rebuilt VerletList (count=" << builds << "), cutsq = " << cutsq
                 << " local size = " << vlPairs.size());
    LOG4ESPP_INFO(theLogger, "rebuilt VerletList (count=" << builds << "), cutsq = " << cutsq
		  << " local size = " << vlPairs.size() << " " << vlPairs1.size()
		  << " " << vlPairs2.size());
  }

  void VerletList::rebuild2()
  {
    //real cutVerlet = cut + getSystem() -> getSkin();
    cutVerlet = cut + getSystem() -> getSkin2();
    cutsq = cutVerlet * cutVerlet;
	...
	if (mass1 + mass2 > 9.) {
	    checkPair2(*it->first, *it->second);
	    LOG4ESPP_DEBUG(theLogger, "checking particles " << it->first->id() << " and " << it->second->id());
	}
    }
    //vlPairs = vlPairs1; //rebuild
    //std::copy(vlPairs2.begin(),vlPairs2.end(),std::back_inserter(vlPairs)); //rebuild
    
    builds2++;
    LOG4ESPP_DEBUG(theLogger, "rebuilt2 VerletList (count=" << builds2 << "), cutsq = "
		   << cutsq << " local size = " << vlPairs.size());
    LOG4ESPP_INFO(theLogger, "rebuilt2 VerletList (count=" << builds << "), cutsq = " << cutsq
		  << " local size = " << vlPairs.size() << " " << vlPairs1.size()
		  << " " << vlPairs2.size());
  }
  ...
  void VerletList::checkPair2(Particle& pt1, Particle& pt2)
  {
    ...
    vlPairs2.add(pt1, pt2); // add pair to Verlet List
  }
  ...
  ****************************************************/
  ...
  void VerletList::registerPython() {
    ...
    class_<VerletList, shared_ptr<VerletList> >
      ...
      .add_property("builds2", &VerletList::getBuilds2, &VerletList::setBuilds2)
