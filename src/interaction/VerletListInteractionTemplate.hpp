...
namespace espressopp {
  namespace interaction {
    ...
    addForces() {
	int vlist_num = verletList->getPairs().size();//add
      LOG4ESPP_DEBUG(_Potential::theLogger, "loop over verlet list pairs and add forces");
      LOG4ESPP_INFO(_Potential::theLogger, "Size of VerletList" << vlist_num);//add
      int num = 0;
	...
	if ((num%1 == 0) && (num > 351100) && (num < 351110)){
          LOG4ESPP_INFO(_Potential::theLogger, num << " id1=" << p1.id() << " id2=" << p2.id());
	}
        Real3D force(0.0);
        ...
        //if(potential->_computeForce(force, p1, p2)) {
          p1.force() += force;
          p2.force() -= force;
          LOG4ESPP_DEBUG(_Potential::theLogger, "id1=" << p1.id() << " id2=" << p2.id() << " force=" << force);//change
        }
	num++;
      }
    }
