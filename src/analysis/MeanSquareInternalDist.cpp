...
namespace espressopp {
  namespace analysis {
    ...
    */
      ...
      boost::unordered_map<long unsigned int, Real3D> pos_temporal;
      ...
      for (int n=1; n<chainlength;n++){ // loop over chainlength
          ...
          for (int j=0; j<N_chains;j++){
              ...
              Real3D pos1 = getConf(M-1)->getCoordinates(localIDs[j*chainlength]);   // compute the MSID of the last added snapshot
              pos_temporal[j*chainlength] = pos1;
              for (int i=j*chainlength; i<(j*chainlength)+chainlength-n;i++){
	              ...
	              pos2 = pos1 + diff;
	              pos1 = pos2;
	              pos_temporal[i + n] = pos2;
                      delta = diff;   //vector with the distances from i to chainlength-n}
                  } else {
                      delta = pos_temporal[i + n] - pos_temporal[i];   //vector with the distances from i to chainlength-n}
                  }
