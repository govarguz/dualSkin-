...
namespace espressopp {
  ...
  namespace integrator {
    ...
    void VelocityVerlet::run(int nsteps)
    {
      ...
      real skin2Half = 0.5 * system.getSkin2();
	...
	LOG4ESPP_INFO(theLogger, "resort2 particles");
        storage.decompose2();
        maxDist2 = 0.0;
        resortFlag2 = false;
      }
      ...
      for (int i = 0; i < nsteps; i++) {
        ...
        LOG4ESPP_DEBUG(theLogger, "maxDist  = " << maxDist  << ", skin/2 = " << skinHalf);
        LOG4ESPP_DEBUG(theLogger, "maxDist2 = " << maxDist2 << ", skin2/2 = " << skin2Half);

        if (maxDist > skinHalf) resortFlag = true;
        if (maxDist2 > skin2Half) resortFlag2 = true; //add
	...
	//**** I added this section for implementing rebuild2.
	if (resortFlag2) {
            VT_TRACER("resort2");
            time = timeIntegrate.getElapsedTime();
            LOG4ESPP_INFO(theLogger, "step " << i << ": resort2 particles");
            storage.decompose2();
            maxDist2  = 0.0;
            resortFlag2 = false;
            nResorts2 ++;
    ...
    real VelocityVerlet::integrate1()
    {
      ...
      real maxSqDist2 = 0.0; // maximal square distance a particle moves //added
      for(CellListIterator cit(realCells); !cit.isDone(); ++cit) {
        real sqDist = 0.0;
        LOG4ESPP_DEBUG(theLogger, "updating first half step of velocities and full step of positions")
	...
	real mass = cit->mass(); //add
        //real dtfm = 0.5 * dt / cit->mass(); //add
        real dtfm = 0.5 * dt / mass; //add
	...
	if (mass != 5.0) { //add
          ...
          maxSqDist2 = std::max(maxSqDist2, sqDist); //add
	}  //add
      } //add
      ...
      real maxAllSqDist2;
      mpi::all_reduce(*system.comm, maxSqDist2, maxAllSqDist2, boost::mpi::maximum<real>());

      LOG4ESPP_INFO(theLogger, "moved " << count << " particles in integrate1" <<
		    ", max move local = " << sqrt(maxSqDist) << ", and " << sqrt(maxSqDist2) <<
		    ", global = " << sqrt(maxAllSqDist) << ", and " << sqrt(maxAllSqDist2));

      //return sqrt(maxAllSqDist); //change
      maxDist  += sqrt(maxAllSqDist); //add
      maxDist2 += sqrt(maxAllSqDist2); //add
    }
